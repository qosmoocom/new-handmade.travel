export const home = {
  menu : {
    arr : [
      {
        title: 'Voyages',
        link : '#'
      },
      {
        title: 'Avis',
        link : '#'
      },
      {
        title: 'Blog',
        link : '#'
      },
      {
        title: 'Contacts',
        link : '#'
      },
      
    ]
  },
  menu_lang : {
    arr : [
      {
        title: 'Ru',
        link : '#',
        current: false
      },
      {
        title: 'En',
        link : '#',
        current: false
      },
      {
        title: 'De',
        link : '#',
        current: false
      },
      {
        title: 'Fr',
        link : '#',
        current: true
      },
      {
        title: 'It',
        link : '#',
        current: false
      },
    ]
  },
  link_for_contact_me : {
    arr: [
      {
        name : 'facebook',
        src: './images/home/header/ico/facebook.svg',
        link: '#',
        alt : 'facebook'
      },
      {
        name : 'telegram',
        src: './images/home/header/ico/telegram.svg',
        link: '#',
        alt : 'telegram'
      },
      {
        name : 'whatsapp',
        src: './images/home/header/ico/whatsapp.svg',
        link: '#',
        alt : 'whatsapp'
      },
    ]
  },
  header_general_title :{
    value : 'Voyage en Ouzbékistan par vos propres mains'
  },
  header_button_title :{
    value : 'Créez votre propre circuit'
  },
  mission_title : {
    value : 'Notre concept',
  },
  mission_comment : {
    value : 'my hand. I made. my travel',
  },
  mission_comment_short : {
    value : 'my hand. I made. my travel',
  },
  mission_text : {
    value : `Handmade.travel rassemble une nouvelle génération de voyageurs à la recherche d'originalité, d'authenticité et d'indépendance. Notre tâche principale est de donner le choix au voyageur : créer son propre voyage de rêve, ou trouver son voyage idéal dans la bibliothèque de voyages de l'auteur.`
  },
  mission_ceo : {
    value : 'Ravshan Turakulov'
  },
  mission_ceo_prof : {
    value: 'CEO handmade.travel'
  },
  tours_title : {
    value : `Des idées d'auteurs pour vos voyages`,
  },
  tours_comment : {
    value : `Des programmes originaux et des itinéraires non conventionnels pour ceux qui n'ont pas peur d'essayer de nouvelles choses dans la vie. Ne vous limitez pas à une idée... Faites en sorte que cela se produise !`,
  },
  tours_comment_short : {
    value : `Des programmes originaux et des itinéraires non conventionnels pour ceux qui n'ont pas peur d'essayer de nouvelles choses dans la vie. Ne vous limitez pas à une idée... Faites en sorte que cela se produise !`,  },
  tours_button_title : {
    value: 'Показать программу'
  },
  tours_view_more_text : {
    value : 'Показать еще'
  },
  utp_title : {
    value : 'Pourquoi handmade.travel?',
  },
  utp_comment : {
    value : `Parce qu'avec nous, le rêve du voyage parfait devient réalité, l'inaccessible devient réalisable et le non-atteignable devient faisable!`,
  },
  utp_comment_short : {
    value : `Parce qu'avec nous, le rêve du voyage parfait devient réalité, l'inaccessible devient réalisable et le non-atteignable devient faisable!`,
  },
  utp_list : {
    arr: [
      {
        title : 'Constructeur VS Bibliothèque des circuits',
        comment: `Pour rendre le voyage accessible à tous ! Choisissez le voyage de vos rêves dans notre bibliothèque de circuits d'auteur ou créez votre propre circuit sur mesure et relaxez-vous selon votre propre scénario.`,
        src: './images/home/utp/ico1.jpg',
        alt : 'Constructeur VS Bibliothèque des circuits',
      },
      {
        title : '"La convivialité, ça nous concerne !"',
        comment: `Nous n'avons pas de touristes, seulement des voyageurs avec lesquels nous sommes sur la même longueur d'onde ! Nous vous garantissons la sécurité financière, nous vous faisons gagner du temps et de l'argent et nous vous donnons une totale liberté d'action et de choix.`,
        src: './images/home/utp/ico1.jpg',
        alt : '"La convivialité, ça nous concerne !"',
      },
      {
        title : `Faites des économies et voyagez en toute indépendance`,
        comment: `Un expert en voyages personnel vous fera gagner jusqu'à 80 % de votre temps, vous aidera à planifier et à organiser le voyage parfait et, si nécessaire, calculera gratuitement le coût de votre voyage en solo. `,
        src: './images/home/utp/ico1.jpg',
        alt : `Faites des économies et voyagez en toute indépendance`,
      },
    ]
  },
  authors_title : {
    value : 'Des experts locaux en voyage',
  },
  authors_comment : {
    value : `L'équipe de handmade.travel est composée de jeunes gens énergiques et créatifs ayant une expérience internationale. Nos auteurs touristiques savent tout sur votre voyage en Ouzbékistan et même un peu plus.  `,
  },
  authors_comment_short : {
    value : `L'équipe de handmade.travel est composée de jeunes gens énergiques et créatifs ayant une expérience internationale. Nos auteurs touristiques savent tout sur votre voyage en Ouzbékistan et même un peu plus.  `,
  },
  authors_button_link : {
    value : 'Написать автору',
  },
  authors_list : {
    arr: [
      {
        name : 'La Belle Catherine',
        info: `Elle est l'auteur de circuits gastronomiques et œnologiques. Avec une spontanéité réfléchie, Catherine rassemble de petits groupes de personnes partageant les mêmes idées pour des voyages non conventionnels dans les paradis culinaires et les vignobles d'Ouzbékistan. `,
        src: './images/home/authors/kate.jpg',
        alt : 'La Belle Catherine',
        telegram : '#',
        facebook : '#',
        whatsapp : '#'
      },
      {
        name : 'Narina-Khanum',
        info: `Spécialiste des voyages artistiques, ceux d'enterrements de vie de jeune fille et des voyages de danse en Ouzbékistan. Elle transmet de la sérotonine, de la dopamine et de l'endorphine. 
        Sur l'étendue de handmade.travel, Narina crée des expériences de voyage qui vous donnent envie de retourner en Ouzbékistan.`,
        src: './images/home/authors/narina.jpg',
        alt : 'Narina-Khanum',
        telegram : '#',
        facebook : '#',
        whatsapp : '#'
      },
      {
        name : 'Fuzuliy, le philosophe',
        info: `Le concepteur de voyages exclusifs en Ouzbékistan : des circuits de coaching et voyages mystiques aux pratiques soufies. Fuzuliy a déjà créé 5656 circuits uniques, organisé 107 voyages et rendu 1005 personnes heureuses !`,
        src: './images/home/authors/fuzuliy.jpg',
        alt : 'Fuzuliy, le philosophe',
        telegram : '#',
        facebook : '#',
        whatsapp : '#'
      },
      {
        name : 'Umida, ou Umi',
        info: `Un expert local des voyages de festival en Ouzbékistan. Toujours sur la même longueur d'onde que le voyageur, Oumida estime qu'il vaut mieux voir une fois de ses propres yeux qu'entendre cent fois, et encore moins cliquer, faire scroller et hésiter. `,
        src: './images/home/authors/no-photo.jpg',
        alt : 'Umida, ou Umi',
        telegram : '#',
        facebook : '#',
        whatsapp : '#'
      },
      {
        name : 'Khousnora, la douce',
        info: `Petit à petit, Khousnora devient le bodhisattva des circuits personnalisés dans la légendaire Volga Gaz-21. "Selfies Every Day", "Nostalgie, quelle folie" et "LoveStory en Ouzbékistan" sont autant de voyages rétro Made By Khousnora.`,
        src: './images/home/authors/husnora.jpg',
        alt : 'Khousnora, la douce',
        telegram : '#',
        facebook : '#',
        whatsapp : '#'
      },
      {
        name : 'Saïdkhon, des mains en or',
        info: `Un brillant représentant de l’addiction au voyage, un chouchou de l'équipe de handmade.travel, un polyglotte et un expert des voyages de loisirs et d'artisanat en Ouzbékistan. À 25 ans, Saïdkhon a déjà réussi à breveter le voyage artisanal " Les mains habiles de l'Ouzbékistan". `,
        src: './images/home/authors/no-photo.jpg',
        alt : 'Saïdkhon, des mains en or',
        telegram : '#',
        facebook : '#',
        whatsapp : '#'
      },
      {
        name : 'Zokir le Grand',
        info: `Dwayne "The Rock" Johnson de l'équipe handmade.travel, auteur de photos expéditions et organisateur de voyages de chasse et de pêche en Ouzbékistan. Chef cuisinier de qualité, Zokir est tout aussi compétent et professionnel dans sa seconde profession. `,
        src: './images/home/authors/no-photo.jpg',
        alt : 'Zokir le Grand',
        telegram : '#',
        facebook : '#',
        whatsapp : '#'
      },
      
    ]
  },
  reviews_list : {
    arr: [
      {
        name :'Тур в Узбекистан на майские праздники',
        info: '',
        text : "Привет из солнечного Узбекистана! Я автор туров на handmade.travel, где собираю небольшие группы единомышленников для небанальных путешествий по кулинарным святыням Узбекистана. Я люблю путешествовать и знакомиться с близкими по духу людьми. Вторая моя страсть – я обожаю вкусно поесть, а гастрономических радостей в Узбекистане, уж поверьте мне, не меньше, чем звёзд на небе!",
        src: 'https://www.youtube.com/watch?v=L_u6B5a7hWE&t=4s',
        link : '#',
      },
      {
        name : 'Альфия, Альбина, Алина и Амир Темур в Узбекистане',
        info: '',
        text : "Привет из солнечного Узбекистана! Я автор туров на handmade.travel, где собираю небольшие группы единомышленников для небанальных путешествий по кулинарным святыням Узбекистана. Я люблю путешествовать и знакомиться с близкими по духу людьми. Вторая моя страсть – я обожаю вкусно поесть, а гастрономических радостей в Узбекистане, уж поверьте мне, не меньше, чем звёзд на небе!",
        src: 'https://www.youtube.com/watch?v=wAc6-tTb2_M&t=4s',
        link : '#',
      },
      {
        name : 'Семья Полковых про свое путешествие в Узбекистан',
        info: '',
        text : "Привет из солнечного Узбекистана! Я автор туров на handmade.travel, где собираю небольшие группы единомышленников для небанальных путешествий по кулинарным святыням Узбекистана. Я люблю путешествовать и знакомиться с близкими по духу людьми. Вторая моя страсть – я обожаю вкусно поесть, а гастрономических радостей в Узбекистане, уж поверьте мне, не меньше, чем звёзд на небе!",
        src: 'https://www.youtube.com/watch?v=3XtFctguxgQ&t=4s',
        link : '#',
      },
      {
        name : 'Наталия и Кристиан об Узбекистане',
        info: '',
        text : "Привет из солнечного Узбекистана! Я автор туров на handmade.travel, где собираю небольшие группы единомышленников для небанальных путешествий по кулинарным святыням Узбекистана. Я люблю путешествовать и знакомиться с близкими по духу людьми. Вторая моя страсть – я обожаю вкусно поесть, а гастрономических радостей в Узбекистане, уж поверьте мне, не меньше, чем звёзд на небе!",
        src: 'https://www.youtube.com/watch?v=KjyKHUJGusk&t=4s',
        link : '#',
      },
    ]
  },
  reviews_title : {
    value : 'Témoignages de nos voyageurs. Des avis réels et des vidéos de nos voyageurs',
  },
  reviews_comment : {
    value : '97 % de nos clients sont entièrement satisfaits de notre rapport qualité-prix, 98 % nous recommandent à leurs amis et connaissances et 99 % de nos voyageurs donnent une note de 10 sur 10 à nos programmes de voyage.',
  },
  reviews_comment_short : {
    value : '97 % de nos clients sont entièrement satisfaits de notre rapport qualité-prix, 98 % nous recommandent à leurs amis et connaissances et 99 % de nos voyageurs donnent une note de 10 sur 10 à nos programmes de voyage.',
  },
  blogs_title : {
    value : 'Blog',
  },
  blogs_comment : {
    value : 'Handmadeburg - est un blog de voyage pour les amateurs de voyages. Articles utiles, conseils et astuces sur les voyages indépendants, rédigés par des blogueurs locaux. Laissez-vous inspirer et entamez vos randonnées !',
  },
  blogs_comment_short : {
    value : 'Handmadeburg - est un blog de voyage pour les amateurs de voyages. Articles utiles, conseils et astuces sur les voyages indépendants, rédigés par des blogueurs locaux. Laissez-vous inspirer et entamez vos randonnées !',
  },
  blogs_button_title : {
    value: 'Показать программу'
  },
  blogs_view_more_text : {
    value : 'Показать еще'
  },
  blogs_item_read_text : {
    value : 'Читать статью'
  },
  footer_info_title : {
    value : 'handmade.travel'
  },
  footer_info_text : {
    value : `Marque de commerce de Silk Road Destinations. 20 ans de succès sur le marché du voyage. Plus de 35 000 voyageurs heureux. Plus de 100 témoignages vidéo. Lauréat du concours To Do 2014.`
  },
  footer_payment : {
    arr : [
      { 
        src : './images/home/footer/master.png',
        alt : 'master card'
      },
      { 
        src : './images/home/footer/union.png',
        alt : 'union pay'
      },
      { 
        src : './images/home/footer/visa.png',
        alt : 'visa'
      },
    ]
  },
  footer_contacts_title :{
    value : 'Contacts:'
  },
  footer_contacts_first_phone_number : {
    value : '+998 66 231 05 48' 
  },
  footer_contacts_first_phone_region : {
    value : 'Ouzbékistan' 
  },
  footer_contacts_second_phone_number : {
    value : '+3 342 284 02 85' 
  },
  footer_contacts_second_phone_region : {
    value : 'France' 
  },
  footer_contacts_mail : {
    value : 'info@handmade.travel'
  },
  footer_contacts_adress : {
    value : 'Place Kuk Saray 1, Samarkand, 140157'
  },
  footer_contats_links : {
    arr : [
      {
        title : 'Telegram',
        link: '#',
      },
      {
        title : 'WhatsApp',
        link: '#',
      },
      {
        title : 'Facebook',
        link: '#',
      },
    ]
  },
  footer_service_title : {
    value : 'Nos services:'
  },
  footer_service_list : {
    arr : [
      {
        name : `Voyages d'auteur`,
        link : '#'
      },
      {
        name : 'Circuits personnalisés',
        link : '#'
      },
      {
        name : 'Départs garantis',
        link : '#'
      },
      {
        name : 'Voyages combinés',
        link : '#'
      },
      {
        name : 'MICE tourisme',
        link : '#'
      },
      {
        name : 'Tourisme responsable',
        link : '#'
      },
    ]
  },
  footer_company_title : {
    value : 'Information utile:'
  },
  footer_company_list : {
    arr : [
      {
        name : 'Notre équipe',
        link : '#'
      },
      {
        name : 'Blog',
        link : '#'
      },
      {
        name : 'Offre publique',
        link : '#'
      },
      {
        name : 'Politique de confidentialité',
        link : '#'
      },
      {
        name : 'Licences et certificats',
        link : '#'
      },
      {
        name : `Qu’en est-il…`,
        link : '#'
      }
    ]
  },
  footer_links : {
    arr : [
      {
        src : './images/home/footer/face.png',
        alt : 'facebook',
        link: "#",
      },
      {
        src : './images/home/footer/insta.png',
        alt : 'instagram',
        link: "#",
      },
      {
        src : './images/home/footer/teleg.png',
        alt : 'telegram',
        link: "#",
      },
      {
        src : './images/home/footer/vk.png',
        alt : 'vkontakte',
        link: "#",
      },
      {
        src : './images/home/footer/youtube.png',
        alt : 'youtube',
        link: "#",
      }
    ]
  },
}