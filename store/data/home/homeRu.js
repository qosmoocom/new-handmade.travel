export const home = {
  menu : {
    arr : [
      {
        title: 'Туры',
        link : '/tours',
        goto : 'tours'
      },
      {
        title: 'Отзывы',
        link : '/reviews',
        goto : 'reviews'
      },
      {
        title: 'Блог',
        link : '/blog',
        goto : 'blog'
      },
      {
        title: 'Контакты',
        link : '/contacts',
        goto : 'contacts'
      },
    ]
  },
  menu_lang : {
    arr : [
      {
        title: 'Ru',
        link : '#',
        current: true
      },
      {
        title: 'En',
        link : '#',
        current: false
      },
      {
        title: 'De',
        link : '#',
        current: false
      },
      {
        title: 'Fr',
        link : '#',
        current: false
      },
      {
        title: 'It',
        link : '#',
        current: false
      },
    ]
  },
  link_for_contact_me : {
    arr: [
      {
        name : 'facebook',
        src: '/images/home/header/ico/facebook.svg',
        link: 'https://www.facebook.com/messages/t/581402519012804/',
        alt : 'facebook'
      },
      {
        name : 'telegram',
        src: '/images/home/header/ico/telegram.svg',
        link: 'https://t.me/handmadetravel_ru',
        alt : 'telegram'
      },
      {
        name : 'whatsapp',
        src: '/images/home/header/ico/whatsapp.svg',
        link: 'https://api.whatsapp.com/send/?phone=998937201028&text&type=phone_number&app_absent=0',
        alt : 'whatsapp'
      },
    ]
  },
  header_general_title :{
    value : 'Тур в Узбекистан своими руками'
  },
  header_button_title :{
    value : 'Создать свой тур'
  },
  mission_title : {
    value : 'Наша концепция',
  },
  mission_comment : {
    value : 'my hand. I made. my travel',
  },
  mission_comment_short : {
    value : 'my hand. I made. my travel',
  },
  mission_text : {
    value : 'Handmade.travel объединяет путешественников нового поколения, ищущих оригинальности, аутентичности и самостоятельности. Наша главная задача - предоставить путешественнику возможность выбора: самостоятельно создать поездку своей мечты, либо подобрать свое идеальное путешествие из Библиотеки авторских туров.'
  },
  mission_ceo : {
    value : 'Ravshan Turakulov'
  },
  mission_ceo_prof : {
    value: 'CEO handmade.travel'
  },
  tours_title : {
    value : 'Авторские идеи для ваших путешествий',
  },
  tours_comment : {
    value : 'Оригинальные программы и небанальные маршруты для тех, кто не боится попробовать новое в жизни. Не ограничивайтесь идеей... Осуществите ее!',
  },
  tours_comment_short : {
    value : 'Оригинальные программы и небанальные маршруты для тех, кто не боится попробовать новое в жизни.',
  },
  tours_button_title : {
    value: 'Показать программу'
  },
  tours_view_more_text : {
    value : 'Показать еще'
  },
  utp_title : {
    value : 'Почему handmade.travel?',
  },
  utp_comment : {
    value : 'Потому что с нами мечта об идеальном путешествии становится реальностью, недоступное - достижимым, а недосягаемое - осуществимым!',
  },
  utp_comment_short : {
    value : 'Потому что с нами мечта об идеальном путешествии становится реальность.',
  },
  utp_list : {
    arr: [
      {
        title : 'Конструктор VS Библиотеки туров',
        comment: 'Делаем путешествия доступным для каждого! Выбери поездку своей мечты из нашей библиотеки авторских туров или создай свой индивидуальный тур и отдохни по собственному сценарию.',
        src: '/images/home/utp/ico1.jpg',
        alt : 'Конструктор',
      },
      {
        title : '"Юзерфрендли”  - это про нас!',
        comment: 'У нас нет туристов, лишь путешественники, с которыми мы на одной волне! Гарантируем сервис и финансовую безопасность, бережем ваше время и деньги, предоставляем полную свободу действий и выбора.',
        src: '/images/home/utp/ico1.jpg',
        alt : 'Конструктор',
      },
      {
        title : 'Экономь и путешествуй самостоятельно',
        comment: 'Личный тревел-эксперт сэкономит до 80% вашего времени, поможет спланировать и организовать идеальное путешествие, а если нужно бесплатно рассчитает стоимость вашей самостоятельной поездки. ',
        src: '/images/home/utp/ico1.jpg',
        alt : 'Конструктор',
      },
    ]
  },
  authors_title : {
    value : 'Локальные тревел-эксперты',
  },
  authors_comment : {
    value : 'Команда handmade.travel - это молодые, энергичные и креативные ребята с международным с опытом работы. Наши авторы туров знают о вашем путешествии в Узбекистан все и чуточку больше.',
  },
  authors_comment_short : {
    value : 'Команда handmade.travel - это молодые, энергичные и креативные ребята.',
  },
  authors_button_link : {
    value : 'Написать автору',
  },
  authors_list : {
    arr: [
      {
        name : 'ГУЛЬЧАТАЙ КАТЯ',
        info: 'Автор гастрономических туров. С продуманной спонтанностью Катя собирает небольшие группы единомышленников для небанальных путешествий по кулинарным святыням и достопримечательностям Узбекистана. ',
        src: '/images/home/authors/kate.jpg',
        alt : 'ГУЛЬЧАТАЙ КАТЯ',
        telegram : 'https://t.me/handmadetravel_ru',
        facebook : 'https://www.facebook.com/messages/t/581402519012804/',
        whatsapp : 'https://wa.me/998937201028'
      },
      {
        name : 'ХУСНОРИК',
        info: 'Маленькая, да удаленькая, Хуснора - бодхисатва индивидуальных авторских туров на легендарной Волга Газ-21. "Селфи каждый день", "Ностальжи, черт побери" и "LoveStory в Узбекистане" - все это ретро-туры Made By Husnora.',
        src: '/images/home/authors/husnora.jpg',
        alt : 'ХУСНОРИК',
        telegram : 'https://t.me/handmadetravel_en',
        facebook : 'https://www.facebook.com/messages/t/100077795766837',
        whatsapp : 'https://wa.me/message/E3FN5F3WIHY6N1'
      },
      {
        name : 'САИДХОН',
        info: 'Яркий представитель "тревел аддикции", любимчик команды handmade.travel, полиглот и гуру хобби-туров и крафтовых путешествий в Узбекистан. В свои 25 Саидхон уже успел запатентовать ремесленный тур "Очумелые ручки по-узбекски".',
        src: '/images/home/authors/said.jpg',
        alt : 'ЗОЛОТЫХ ДЕЛ МАСТЕР САИДХОН',
        telegram : 'https://t.me/handmade_travel',
        facebook : 'https://www.facebook.com/profile.php?id=100077955722581',
        whatsapp : 'https://wa.me/qr/2ZMJNAQZJPHMN1'
      },
      {
        name : 'ЗОКИР ПОПОВИЧ',
        info: 'Дуэйн "Скала" Джонсон команды handmade.travel, автор фото-экспедиций, организатор охот и рыбалок в Узбекистане. Зокир не менее профессионален и компетентен в своей второй профессии.',
        src: '/images/home/authors/zokir.jpg',
        alt : 'ЗОКИР ПОПОВИЧ',
        telegram : '#',
        facebook : '#',
        whatsapp : '#'
      },
      {
        name : 'ФИЛОСОФ ФУЗУЛИЙ',
        info: 'Сочинитель авторских путешествий в Узбекистан: от коучинг тураов до мистических сабантуев и суфийских практик. Фузулий уже успел накреативить 5656 уникальных туров, организовал 107 путешествий и сделал счастливыми 1005 людей!',
        src: '/images/home/authors/fuzuliy.jpg',
        alt : 'ФИЛОСОФ ФУЗУЛИЙ',
        telegram : 'https://t.me/handmadetravelde',
        facebook : 'https://www.facebook.com/messages/t/100076785810304',
        whatsapp : 'https://wa.me/998937201032'
      },
      {
        name : 'НАРИНА-ХАНУМ',
        info: 'Мата-хари арт-туров, девичников и dance вояжей в Узбекистан. Дарит серотонин, дофамин и эндорфин. На просторах handmade.travel Нарина чудит и творит путешествия-впечатления, после которых хочется вернуться в Узбекистан.',
        src: '/images/home/authors/narina.jpg',
        alt : 'НАРИНА-ХАНУМ',
        telegram : 'https://t.me/handmadetravel_fr',
        facebook : 'https://www.facebook.com/handmadetravelfr-371849660089376',
        whatsapp : 'https://wa.me/message/OZVYJHI5QOLFJ1'
      },
      {
        name : 'УМИДА, ОНА ЖЕ УМИ',
        info: 'Локальный эксперт по фестивальным турам в Узбекистан. Всегда на одной волне с путешественником, Умида уверена, что лучше один раз увидеть собственными глазами, чем сто раз услышать и уж тем более кликать, скролить и сомневаться.',
        src: '/images/home/authors/umi.jpg',
        alt : 'УМИДА, ОНА ЖЕ УМИ',
        telegram : '#',
        facebook : '#',
        whatsapp : '#'
      },
      
      
      
    ]
  },
  reviews_list : {
    arr: [
      {
        name :'Тур в Узбекистан на майские праздники',
        img: '/images/home/reviews/ru-1.jpg',
        text : "Если честно мы даже не ожидали, что Узбекистан произведет на нас такое впечатление. Все мы с детства и юности мечтали увидеть Бухару и Самарканд. И только сейчас смогли приехать. Древняя цивилизация и культура, люди очень добрые, отзывчивые, веселые, солнце, колоритные базары, необыкновенные музеи ... ",
        src: 'https://www.youtube.com/watch?v=L_u6B5a7hWE&t=4s',
        link : '#',
      },
      {
        name : 'Альфия, Альбина, Алина и Амир Темур в Узбекистане',
        img: '/images/home/reviews/ru-2.jpg',
        text : "Все прошло на высшем уровне! Были учтены все пожелания, был очень тонкий подход ко всему, действовали быстро и оперативно. Программа была настолько интересной, что нам захотелось приехать еще раз! Также хотелось бы поблагодарить нашего гида Саида! Он очень эрудирован, увлекательный рассказчик, очень внимательный, воспитанный и …",
        src: 'https://www.youtube.com/watch?v=wAc6-tTb2_M&t=4s',
        link : '#',
      },
      {
        name : 'Семья Полковых про свое путешествие в Узбекистан',
        img: '/images/home/reviews/ru-3.jpg',
        text : "Вопрос ехать или не не ехать в Узбекистан, тут просто не стоит. Конечно, ехать! Потому что страна красивая, страна древняя, города - ровесники Рима, а может даже и старше. Однако главное богатство Узбекистана, это не природа, архитектура, или климат. Главное богатство - это люди! Маленькая ли это деревня или столица Ташкент ...",
        src: 'https://www.youtube.com/watch?v=3XtFctguxgQ&t=4s',
        link : '#',
      },
      {
        name : 'Наталия и Кристиан об Узбекистане',
        img: '/images/home/reviews/ru-4.jpg',
        text : "Конечно, не секретом будет, что мы ожидали увидеть немножко не то. Предполагали,  что здесь может быть небезопасно, могут возникнуть проблемы в путешествии…В общем, были сомнения, которые развеялись сразу же, как только мы приехали в Узбекистан. На самом деле, ситуация была совершенно иная. Все было очень хорошо, безопасно ...",
        src: 'https://www.youtube.com/watch?v=KjyKHUJGusk&t=4s',
        link : '#',
      },
    ]
  },
  reviews_title : {
    value : 'Реальные отзывы и видео-отчеты',
  },
  reviews_comment : {
    value : '97% наших гостей полностью устраивает соотношение цена/качество, 98% рекомендуют нас своим друзьям и знакомым, а 99% наших путешественников оценивают наши авторские программы на 10 из 10.',
  },
  reviews_comment_short : {
    value : '97% наших гостей полностью устраивает соотношение цена/качество, 98% рекомендуют нас своим друзьям и знакомым.',
  },
  blogs_title : {
    value : 'Блог',
  },
  blogs_comment : {
    value : 'Handmadeburg - это travel блог для охотников за путешествиями. Полезные статьи, советы и лайфхаки по самостоятельной поездке от локальных блогеров. Вдохновляйтесь и начинайте паковать рюкзаки!',
  },
  blogs_comment_short : {
    value : 'Handmadeburg - это travel блог для охотников за путешествиями. Полезные статьи, советы и лайфхаки по самостоятельной поездке от локальных блогеров.',
  },
  blogs_button_title : {
    value: 'Показать программу'
  },
  blogs_view_more_text : {
    value : 'Показать еще'
  },
  blogs_item_read_text : {
    value : 'Читать статью'
  },
  footer_info_title : {
    value : 'handmade.travel'
  },
  footer_info_text : {
    value : 'торговый знак Silk Road Destinations. 20 лет успеха на туристическом рынке более 35 тысяч счастливых туристов свыше 200 видео-отзывов победитель конкурса To Do 2014'
  },
  footer_payment : {
    arr : [
      { 
        src : '/images/home/footer/master.png',
        alt : 'master card'
      },
      { 
        src : '/images/home/footer/union.png',
        alt : 'union pay'
      },
      { 
        src : '/images/home/footer/visa.png',
        alt : 'visa'
      },
    ]
  },
  footer_contacts_title :{
    value : 'Контакты:'
  },
  footer_contacts_first_phone_number : {
    value : '+998 66 231 05 48' 
  },
  footer_contacts_first_phone_region : {
    value : 'Узбекистан' 
  },
  footer_contacts_second_phone_number : {
    value : '+7 812 309 52 11' 
  },
  footer_contacts_second_phone_region : {
    value : 'Россия' 
  },
  footer_contacts_mail : {
    value : 'russian@handmade.travel'
  },
  footer_contacts_adress : {
    value : 'Площадь Кук Сарай 1, Самарканд, 140157'
  },
  footer_contats_links : {
    arr : [
      {
        title : 'Telegram',
        link: 'https://t.me/handmadetravel_ru',
      },
      {
        title : 'WhatsApp',
        link: 'https://api.whatsapp.com/send/?phone=998937201028&text&type=phone_number&app_absent=0',
      },
      {
        title : 'Facebook',
        link: 'https://www.facebook.com/messages/t/581402519012804/',
      },
    ]
  },
  footer_service_title : {
    value : 'Наши услуги:'
  },
  footer_service_list : {
    arr : [
      {
        name : 'Авторские туры',
        link : '#'
      },
      {
        name : 'Индивидуальные туры',
        link : '#'
      },
      {
        name : 'Гарантированные туры',
        link : '#'
      },
      {
        name : 'Комбинированные туры',
        link : '#'
      },
      {
        name : 'MICE туризм',
        link : '#'
      },
      {
        name : 'Ответственный туризм',
        link : '#'
      },
    ]
  },
  footer_company_title : {
    value : 'Полезная информация:'
  },
  footer_company_list : {
    arr : [
      {
        name : 'Наша команда',
        link : '#'
      },
      {
        name : 'Блог',
        link : '#'
      },
      {
        name : 'Публичная оферта',
        link : '/offer'
      },
      {
        name : 'Политика конфиденциальности',
        link : '/politic'
      },
      {
        name : 'Лицензии и сертификаты',
        link : '#'
      },
      {
        name : 'ЧаВо',
        link : '/what'
      }
    ]
  },
  footer_links : {
    arr : [
      {
        src : '/images/home/footer/face.png',
        alt : 'facebook',
        link: "https://www.facebook.com/handmadetravelru-581402519012804",
      },
      {
        src : '/images/home/footer/insta.png',
        alt : 'instagram',
        link: "https://www.instagram.com/handmade.travel_ru/",
      },
      {
        src : '/images/home/footer/teleg.png',
        alt : 'telegram',
        link: "https://t.me/handmade_travel_ru",
      },
      {
        src : '/images/home/footer/vk.png',
        alt : 'vkontakte',
        link: "https://vk.com/id687844599",
      },
      {
        src : '/images/home/footer/youtube.png',
        alt : 'youtube',
        link: "https://www.youtube.com/channel/UCsi-g38QfxcXb4_N-vJghhg",
      }
    ]
  },
}